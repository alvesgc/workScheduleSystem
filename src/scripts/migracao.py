...
if num_registros_antes > num_registros_depois:
    ...
    print(
        f"AVISO: {removidos} matrícula(s) duplicada(s) foi(ram) encontrada(s) e ignorada(s). Apenas o primeiro registro de cada foi mantido."
    )
...
try:
    ...
    colunas_para_db = [
        col for col in mapeamento_colunas.values() if col in df_ativos.columns
    ]
