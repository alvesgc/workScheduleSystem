...
def exportar_para_pdf(dados_escala, ano, mes, caminho_arquivo):
    ...
    elementos = [] # Lista principal de elementos do PDF

    # --- Estilos de Parágrafo (para Células e Títulos) ---
    ...
    # --- Definições Globais da Tabela (Cabeçalhos e Larguras) ---
    ...
    # --- Estilos de Base (Reutilizados por todas as tabelas) ---
    estilos_base = [
    ...
    # Estilos de FDS para os Cabeçalhos (Linhas 0 e 1)
    estilos_fds_cabecalho = []
    for dia_num in range(1, num_dias + 1):
        if weekday(ano, mes, dia_num) >= 5:
            col_index = dia_num + 3 # Offset de 4 colunas
            estilos_fds_cabecalho.append(
                ("BACKGROUND", (col_index, 0), (col_index, 1), colors.HexColor("#E7E6E6"))
            )

    # --- MUDANÇA PRINCIPAL: Loop para criar uma tabela por setor ---
    for setor_nome, escalas_do_setor in sorted(grupos_de_setor.items()):

        # 1. Inicia os dados e estilos para ESTA tabela
        ...
        # Loop pelas ESCALAS dentro do Setor
        for i, (escala_nome_grupo, colaboradores_do_grupo) in enumerate(sorted(
            escalas_do_setor.items()
        )):
            
            # --- LINHA DE TÍTULO (Setor e Escala) ---
            titulo_setor_cell = ""
            if i == 0: # Só mostra o nome do SETOR na primeira escala
                titulo_setor_cell = Paragraph(setor_nome, style_setor_row)

            titulo_escala_cell = Paragraph(
                escala_nome_grupo.upper(), style_escala_row
            )
            
            linha_titulo = (
                [titulo_setor_cell, "", "", "", titulo_escala_cell]
                + [""] * (colunas_totais - 5)
            )
            dados_para_esta_tabela.append(linha_titulo)

            # Estilos para a linha de Título
            cor_fundo_titulo = colors.HexColor("#C6E0B4") if i == 0 else colors.HexColor("#E2EFD9")
            ...
            estilos_para_esta_tabela.append(('LEFTPADDING', (0, row_index), (0, row_index), 10))
            estilos_para_esta_tabela.append(('RIGHTPADDING', (4, row_index), (4, row_index), 10))
            estilos_para_esta_tabela.append(('VALIGN', (0, row_index), (-1, row_index), "MIDDLE"))
            
            # Garante que o título e a primeira linha de dados fiquem juntos
            estilos_para_esta_tabela.append(('NOSPLIT', (0, row_index), (-1, row_index + 1)))
            row_index += 1
            ...
            for matricula, info, _ in dados_ordenados_grupo:
                ...
                estilos_para_esta_tabela.append(("TOPPADDING", (0, row_index), (-1, row_index), 3))
                estilos_para_esta_tabela.append(("BOTTOMPADDING", (0, row_index), (-1, row_index), 3))

                # Cor de FDS para esta linha de dados
                for dia_num in range(1, num_dias + 1):
                    if weekday(ano, mes, dia_num) >= 5:
                        col_index = dia_num + 3
                        estilos_para_esta_tabela.append(
        ...
        # 2. Cria o objeto Tabela para este SETOR
        tabela_setor = Table(dados_para_esta_tabela, colWidths=larguras_colunas)
        
        # 3. Aplica todos os estilos (base + dinâmicos)
        tabela_setor.setStyle(TableStyle(estilos_para_esta_tabela))
        
        # 4. Adiciona a Tabela e o Espaçador ao PDF, envoltos em KeepTogether
