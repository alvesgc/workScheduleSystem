...
class GeradorEscalaEngine:
    ...
    def _gerar_ciclo(self, data_base, horas_trabalho, horas_folga):
        ...
        try:
            ...
            data_atual = datetime.combine(date.fromisoformat(str(data_base)), datetime.min.time().replace(hour=7))
        except (ValueError, TypeError):
            return []

        # Avança a data até o período de geração
        limite_inferior = datetime(self.ano, self.mes, 1) - (intervalo * 5) # Buffer de 5 ciclos
        ...
        try:
            ...
            limite_superior = datetime(self.ano, self.mes, monthrange(self.ano, self.mes)[1]) + timedelta(days=1)
    ...
    def _calcular_escala_24x120(self, colaborador):
        """Calcula a escala 24x120 com a lógica de par/ímpar e atualiza o estado."""
        data_base = colaborador.get('escala_data_base')
        sequencia_anterior = colaborador.get('escala_sequencia_atual', 'IMPAR')
        matricula = colaborador.get('matricula')
        if not data_base or not matricula: return []

        sequencia_deste_mes = 'PAR' if sequencia_anterior == 'IMPAR' else 'IMPAR'
        
        turnos_formatados = []
        for dt_inicio_turno in self._gerar_ciclo(data_base, 24, 120):
            dia = dt_inicio_turno.day
            if (sequencia_deste_mes == 'PAR' and dia % 2 == 0) or \
               (sequencia_deste_mes == 'IMPAR' and dia % 2 != 0):
                turnos_formatados.append({'dia': dia, 'turno': '24h'})
    ...
    def executar(self):
        colaboradores = db.get_all_active_collaborators()
