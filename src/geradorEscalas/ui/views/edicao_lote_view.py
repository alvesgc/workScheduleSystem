...
class EdicaoEmLoteView(ctk.CTkFrame):
    def __init__(self, master, app_controller, matriculas):
        ...
        ctk.CTkLabel(header_frame, text=f"Editando {len(matriculas)} Colaboradores em Lote", font=fonts.TITULO_SECAO).pack(anchor="w")
        ...
        # Busca os nomes no banco para exibir
        for mat in self.matriculas:
            colab = db.get_collaborator_by_matricula(mat)
            if colab:
                ctk.CTkLabel(scrollable_list, text=colab.get('nome', '')).pack(anchor="w", padx=10)
