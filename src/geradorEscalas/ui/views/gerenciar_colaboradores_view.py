...
# A linha 'from tktooltip import ToolTip' foi removida.
...
class GerenciarColaboradoresView(ctk.CTkFrame):
    def __init__(self, master, app_controller, data_to_load=None):
        ...
        # As linhas 'ToolTip(...)' foram removidas.
    ...
    def update_table(self, search_term=None, invalid_rows=None):
        ...
        if df.empty:
            ...
            empty_label = ctk.CTkLabel(self.table_frame, text=msg_text, font=("", 16), text_color="gray60")
        ...
        self.tree.tag_configure('hover', background='#2A2D2E')
        ...
        for col in columns:
            ...
            if col in ['Nome', 'nome']: width = 300
            elif col in ['Cargo', 'cargo']: width = 250
            elif col in ['Setor', 'setor']: width = 200
            else: width = 120
        ...
        for i, record in enumerate(self.table_data):
            ...
            self.tree.insert("", "end", iid=iid, image=self.img_unchecked, values=list(record.values()), tags=(tag,))
