...
class GerenciarColaboradoresView(ctk.CTkFrame):
    def __init__(self, master, app_controller, data_to_load=None):
        ...
        icon_import = fa.icon_to_image(
            "file-import", fill=icon_color, scale_to_height=16
        )
        ...
        header_frame.grid_columnconfigure(
            1, weight=1
        )  # Cria espaço entre o título e os botões

        ctk.CTkLabel(
            header_frame, text="Gerenciar Colaboradores", font=fonts.TITULO_SECAO
        ...
        self.table_frame = ctk.CTkFrame(
            self, fg_color="#2B2B2B", border_width=1, border_color="gray30"
        )
    ...
    def edit_selected(self):
        ...
        if len(matriculas_selecionadas) == 1:
            # Chama diretamente o método do app_controller para a edição única
            self.app_controller.show_cadastro_manual_view(matricula_para_editar=matriculas_selecionadas[0])
        else: # > 1
            # Coleta os dados completos para a edição em lote
            dados_selecionados = []
            ...
            # Chama diretamente o método do app_controller para a edição em lote
