...
class SetupEscalaView(ctk.CTkToplevel):
    def __init__(self, master, colaboradores, save_callback):
        ...
        ctk.CTkLabel(
            self, text="Primeira Geração de Escala", font=fonts.TITULO_SECAO
        ).pack(pady=10)
        ctk.CTkLabel(
            self,
            text="Confirme ou ajuste a data de referência para o início do ciclo de cada colaborador.",
            justify="left",
        ).pack(pady=5, padx=20)
        ...
        icon_color = self.cget("fg_color")  # Pega a cor de texto do botão
        icon_calendar = fa.icon_to_image(
            "calendar-alt", fill=icon_color, scale_to_height=16
        )
        ...
        for colab in self.colaboradores:
            ...
            ctk.CTkLabel(
                row_frame, text=f"{nome}:", width=300, anchor="w", font=fonts.LABEL_FONT
            ...
            select_date_button = ctk.CTkButton(
                ...
                width=35,
                height=35,
                fg_color=self.PRIMARY,  # Usa a cor azul do tema
                hover_color=self.PRIMARY_HOVER,
                command=lambda var=date_var: self._open_calendar(var),
            )
            select_date_button.pack(side="left")

        save_button = ctk.CTkButton(
            self, text="Salvar Datas e Continuar", command=self._on_save, height=40
        )
