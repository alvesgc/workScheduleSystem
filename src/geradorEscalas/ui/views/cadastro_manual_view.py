...
class CadastroManualView(ctk.CTkFrame):
    def __init__(self, master, save_callback, back_callback, matricula_para_editar=None):
        ...
        for i, (label, var) in enumerate(self.campos.items()):
            ctk.CTkLabel(scrollable_frame, text=f"{label}:", font=fonts.LABEL_FONT).grid(row=i, column=0, sticky="w", padx=20, pady=10)
            
            if label == "Escala":
                entry = ctk.CTkComboBox(scrollable_frame, variable=var, values=["12x36", "Diarista"], state="readonly", height=35, font=fonts.TEXTO_NORMAL)
            elif label == "Tipo de Turno":
                entry = ctk.CTkComboBox(scrollable_frame, variable=var, values=["Diurno 1", "Diurno 2", "Noturno 1", "Noturno 2", "-"], height=35, font=fonts.TEXTO_NORMAL)
            else:
