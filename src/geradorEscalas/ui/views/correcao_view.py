...
class CorrecaoView(ctk.CTkToplevel):
    def __init__(self, master, row_data, index):
        ...
        ctk.CTkLabel(
            main_frame,
            text=f"Corrigir Colaborador (Linha {index})",
            font=fonts.TITULO_SECAO,
        ).grid(row=0, column=0, pady=(0, 10))
        ctk.CTkLabel(
            main_frame,
            text="Um ou mais campos obrigatórios estão vazios. Por favor, corrija para continuar.",
            wraplength=450,
        ...
        for i, (label, var) in enumerate(self.campos.items()):
            ...
            clean_value = (
                ""
                if pd.isna(original_value) or str(original_value).lower() == "nan"
                else str(original_value)
            )
            var.set(clean_value)

            ctk.CTkLabel(form_frame, text=f"{label}:", font=fonts.LABEL_FONT).grid(
                row=i, column=0, sticky="w", padx=20, pady=10
            )
            entry = ctk.CTkEntry(
                form_frame, textvariable=var, height=35, font=fonts.TEXTO_NORMAL
            )

            # --- LÓGICA DA BORDA VERMELHA E PLACEHOLDER ---
            if label in required_fields and not var.get():
                entry.configure(
                    border_color="#D63031", placeholder_text="* Campo Obrigatório"
                )
        ...
        ctk.CTkButton(button_frame, text="Salvar e Continuar", command=self._save).pack(
            side="left", padx=10
        )
        ctk.CTkButton(
            button_frame,
            text="Pular Colaborador",
            command=self._skip,
            fg_color="#7A7A7A",
        ).pack(side="left")
