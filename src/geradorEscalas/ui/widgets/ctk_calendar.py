...
class CTkCalendar(ctk.CTkToplevel):
    def __init__(self, master, current_date=None, callback=None):
        ...
        # 1. Adicionada a lista de meses em português
        self.month_names_pt = [
    ...
    def _create_widgets(self):
        ...
        self.prev_month_button = ctk.CTkButton(header_frame, text="<", width=30, command=self._prev_month)
        self.prev_month_button.pack(side="left", padx=5)

        self.month_year_label = ctk.CTkLabel(header_frame, text="", font=fonts.SUBTITULO)
        self.month_year_label.pack(side="left", padx=10)

        self.next_month_button = ctk.CTkButton(header_frame, text=">", width=30, command=self._next_month)
        ...
        for i, day in enumerate(weekdays):
            ctk.CTkLabel(weekdays_frame, text=day, font=fonts.LABEL_FONT, width=35).grid(row=0, column=i, padx=2, pady=2)
    ...
    def _update_calendar(self):
        ...
        # 2. Usa a lista para buscar o nome do mês em português
        month_name = self.month_names_pt[self.selected_date.month - 1] # -1 porque a lista é 0-indexed
        ...
        for day_num in range(1, num_days + 1):
            ...
            day_button = ctk.CTkButton(
                ...
                width=35, height=35, fg_color=button_color, text_color=text_color,
    ...
    def _next_month(self):
        last_day_of_current_month = self.selected_date.replace(day=monthrange(self.selected_date.year, self.selected_date.month)[1])
        self.selected_date = (last_day_of_current_month + timedelta(days=1)).replace(day=1)
        self._update_calendar()
